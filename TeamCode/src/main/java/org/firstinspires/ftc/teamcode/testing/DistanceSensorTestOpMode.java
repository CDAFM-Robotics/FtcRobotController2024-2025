...
public class DistanceSensorTestOpMode extends LinearOpMode {
  ...
  public void runOpMode() {
    ...
    while(opModeIsActive()) {
      ...
      double currentValueL = distanceSensorLeft.getDistance(DistanceUnit.INCH);  // imaginary, noisy sensor
      double estimateL = filterLeft.estimate(currentValueL); // smoothed sensor

      double currentValueR = distanceSensorLeft.getDistance(DistanceUnit.INCH);  // imaginary, noisy sensor
      ...
      telemetry.addData("range left", String.format("%.01f in", estimateL));

      telemetry.addLine("");

      telemetry.addLine("Color Sensor Right:");
      telemetry.addData("range right", String.format("%.01f in", /*distanceSensorRight.getDistance(DistanceUnit.MM)*/ estimateR));
      telemetry.update();
    }
  }
}
